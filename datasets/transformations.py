...
class ConvertKeypoints(object):
    def __call__(self, sample):
        ...
        # keypoint[2] == 0: occluded, == 1: visible, == 2: not in image
...
class Rotate(object):
    ...
    def __call__(self, sample):
        ...
        sample['image'] = cv2.warpAffine(sample['image'], R, \
            dsize=dsize, borderMode=cv2.BORDER_CONSTANT, borderValue=self._pad)
        sample['label']['img_height'], sample['label']['img_width'] = sample['image'].shape[:2]
        sample['mask'] = cv2.warpAffine(sample['mask'], R, \
            dsize=dsize, borderMode=cv2.BORDER_CONSTANT, borderValue=(1, 1, 1))  # border is ok
        label = sample['label']
...
class Flip(object):
    ...
    def __call__(self, sample):
        ...
        for other_annotation in label['processed_other_annotations']:
            ...
            for keypoint in other_annotation['keypoints']:
                keypoint[0] = w - 1 - keypoint[0]
            other_annotation['keypoints'] = \
                self._swap_left_right(other_annotation['keypoints'])
        return sample
